...
import { Bot, CheckCircle2, Cpu, Mic, Sparkles, Square } from "lucide-react";
...
export const Route = createFileRoute("/riccos")({
  head: () => ({
    meta: [
      { title: "RiccOS — Central" },
      { name: "description", content: "Robô assistente futurista por voz no RiccOS." },
    ],
  }),
...
function RiccOSFuturisticRobotPage() {
  ...
  const sendAudioToWebhook = async () => {
    try {
      ...
      setState("success");
      // Voltar ao estado de idle automaticamente após 4 segundos
      setTimeout(() => {
        setState("idle");
      }, 4000);
    } catch (err: any) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-6rem)] flex-col items-center justify-center relative overflow-hidden select-none">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div
          className={`size-[500px] rounded-full blur-3xl transition-all duration-700 opacity-20 ${state === "recording"
      ...
      <div className="relative z-10 flex flex-col items-center justify-center">
        ...
        <div className="relative flex items-center justify-center cursor-pointer group" onClick={handleRobotClick}>
          ...
          <div
            className={`absolute size-72 sm:size-80 rounded-full border border-dashed transition-all duration-700 ${state === "recording"
          ...
          <div
            className={`absolute size-60 sm:size-64 rounded-full border transition-all duration-500 ${state === "recording"
          ...
          {/* Anel Interno Neon */}
          <div
            className={`absolute size-48 sm:size-52 rounded-full border-2 transition-all duration-500 ${state === "recording"
          ...
          <div
            className={`relative z-20 flex size-36 sm:size-40 items-center justify-center rounded-full transition-all duration-500 shadow-2xl backdrop-blur-xl ${state === "recording"
          ...
          >
            {state === "recording" ? (
              ...
              <div className="flex flex-col items-center gap-1 group-hover:scale-110 transition-transform">
                <Bot className="size-14 text-cyan-300 drop-shadow-[0_0_15px_rgba(34,211,238,0.8)]" />
